...
const Search = () => {
    ...
    const filterdata = vastu.filter((chani) =>
        chani.title.toLowerCase().includes(term.toLowerCase())
    )

    return (
        <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-black flex flex-col items-center justify-start p-8">
            {filterdata.length === 0 ? (
                <h2 className="text-3xl text-rose-400 font-bold mt-32">Nothing found</h2>
            ) : (
