...
const Productbycategory = () => {
  ...
  const filterfilm = film.filter((chani) => chani.category === `${category}`)
  const filterdata = vastu.filter((chani) => chani.category === `${category}`)

  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-black flex flex-col items-center justify-start py-8">
      {/* Video Section */}
      {filterfilm.map((data) => (
        <div key={data.name} className="w-full flex items-center justify-center mb-10">
          <video
            src={data.url}
            autoPlay
            muted
            loop
            className="rounded-3xl shadow-2xl border-4 border-yellow-400 w-full max-w-5xl object-cover"
          ></video>
        </div>
      ))}
