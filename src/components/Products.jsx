...
const Products = () => {
  ...
  const filtervastu =
    buttoncat === "all"
      ? vastu
      : vastu.filter((chani) => chani.category === buttoncat);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-black flex flex-wrap gap-16 items-center justify-center p-8">
      <div className="w-full flex flex-wrap gap-4 sm:gap-6 items-center justify-center mb-8">
        {buttons.map((data) => (
          <button
            onClick={() => setButtoncat(data.name.toLowerCase())}
            key={data.id}
            className={`border border-rose-300 rounded-3xl w-28 sm:w-32 h-10 font-semibold transition-all duration-200
        ${
          buttoncat === data.name.toLowerCase()
            ? "bg-rose-300 text-red-900 shadow-md scale-105"
            : "bg-transparent text-rose-200 hover:bg-rose-300 hover:text-red-900"
        }
      `}
          >
            {data.name}
      ...
      {filtervastu.map((data) => (
        ...
        >
          ...
          <h1 className="w-full text-center mt-6 text-white font-semibold text-lg truncate">
            {data.title}
          </h1>
          <h1 className="w-full text-center text-yellow-400 font-bold text-xl mt-2 mb-2">
            ₹ {data.price}
          </h1>
          <button
            onClick={() =>
              addtocart(data.id, data.title, data.price, data.imgSrc)
            }
          ...
          >
            ...
            >
              <path
                ...
                d="M2.25 3h1.386c.51 0 .955.343 1.087.835l.383 1.437m0 0L7.5 15.75A2.25 2.25 0 009.683 18h4.634a2.25 2.25 0 002.183-1.813l1.285-7.713a1.125 1.125 0 00-1.107-1.312H6.272m0 0L5.25 4.5m1.022 1.272h13.478"
              />
            </svg>
            Add to Cart
          </button>
